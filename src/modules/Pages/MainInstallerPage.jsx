...
export const MainInstallerPage = () => {
    ...
    const [postError, setPostError] = useState(null); // For POST errors
            ...
            try {
                ...
                setOrders(
                    data.orders.map((order) => ({
                        ...order,
                        id: String(order.id || ''), // Fallback for missing id
                    })) || []
                );
                setInstallers(
    ...
    const postData = async (orderId) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || `Ошибка ${response.status}: Не удалось подтвердить заказ`);
            }
                                        ...
                                        <tr key={order.id}>
                                            ...
                                            <td>
                                                <button
                                                    ...
                                                    disabled={!selectedTag[order.id]} // Disabled when "Выбрать установщика" is selected
                                                >
                                                    Подтвердить
                                                </button>
                                            </td>
